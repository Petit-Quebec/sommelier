...
fn function_handler_helper(event: &Request) -> Result<String, StatusCode> {
    ...
    let application_public_key: [u8; PUBLIC_KEY_LENGTH] = 
        hex::decode(&env!("NYOOMIO_PUBLIC_KEY"))
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
        .try_into()
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    let verifier = 
        VerifyingKey::from_bytes(&application_public_key)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let signature_str = 
        event.headers()
        .get("X-Signature-Ed25519").ok_or(StatusCode::BAD_REQUEST)?
        .to_str().map_err(|_| StatusCode::BAD_REQUEST)?;
